...
# load("GSE59491_expr.RData")

## 读入基因表达数据
expSet <- read.table("GSE59491_series_matrix.txt", header=T, sep='\t', fill=TRUE, strip.white = T)
expSet$ID_REF <- as.character(expSet$ID_REF)  # 将ID_REF列全部转换成符号,为了同anno2合并
# View(expSet[,1:2])

## 读入芯片探针注释文件
anno <- read.table("GPL18964.txt",header=T,sep='\t',fill=TRUE,strip.white = T,quote = "")  # 探针和基因ID的对应文件
# View(anno[,1:2])
...
dim(expset4)    # 24478   327
# write.table(expset4,"GSE59491_expr.txt",quote=F,sep="\t")   
# write.table(expset4,"GSE59491_expr_name.txt",quote=F,sep="\t")    # 行名为基因   

# 标签 ----------------------------------------------------------------------

lable2 = read.csv("GSE59491_all.csv", header = T, sep=',')
dim(lable2)    # 326   2
data = rbind(as.matrix(t(lable2[,2])), as.matrix(expset4))
rownames(data) <- c('Lable', rownames(expset4))
View(data[1:10,1:10])
dim(data)    #  24479   326
write.table(data,"GSE59491_outcome.txt",quote=F,sep="\t")
